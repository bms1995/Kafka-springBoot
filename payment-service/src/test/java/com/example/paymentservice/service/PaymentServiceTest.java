...
class PaymentServiceTest {
  ...
  void processPaymentPublishesSuccessEventAndMarksOrderAsProcessed() {
    ...
    verify(paymentOutboxService)
        .enqueuePaymentProcessed(
            org.mockito.ArgumentMatchers.eq(event.getOrderId()), paymentEventCaptor.capture());
  ...
  void processPaymentSkipsAlreadyProcessedOrder() {
    ...
    verify(paymentOutboxService, never())
        .enqueuePaymentProcessed(
  ...
  void processPaymentPublishesFailureEventWhenAmountExceedsThreshold() {
    ...
    verify(paymentOutboxService)
        .enqueuePaymentFailed(
            org.mockito.ArgumentMatchers.eq(event.getOrderId()), failedEventCaptor.capture());
    verify(paymentTransactionRepository).save(transactionCaptor.capture());
    verify(processedEventRepository).save(processedEventCaptor.capture());
  ...
  void compensatePaymentPublishesRefundEvent() {
    ...
    PaymentTransaction transaction =
        new PaymentTransaction(event.getOrderId(), BigDecimal.valueOf(250), "customer@example.com");
    ...
    verify(processedEventRepository).save(processedEventCaptor.capture());
    verify(paymentOutboxService)
        .enqueuePaymentRefunded(
            org.mockito.ArgumentMatchers.eq(event.getOrderId()), refundedEventCaptor.capture());
