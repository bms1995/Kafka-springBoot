...
class RateLimitingFilterTest {
  ...
  void allowsRequestsWithinLimit() {
    RateLimitingFilter filter =
        new RateLimitingFilter(new RateLimitProperties(true, 2, Duration.ofMinutes(1)), clock);
  ...
  void rejectsRequestsAboveLimit() {
    RateLimitingFilter filter =
        new RateLimitingFilter(new RateLimitProperties(true, 1, Duration.ofMinutes(1)), clock);
    ...
    assertThat(
            rejected
                .getResponse()
                .getHeaders()
                .getFirst(RateLimitingFilter.RATE_LIMIT_LIMIT_HEADER))
        .isEqualTo("1");
    assertThat(
            rejected
                .getResponse()
                .getHeaders()
                .getFirst(RateLimitingFilter.RATE_LIMIT_REMAINING_HEADER))
        .isEqualTo("0");
  }
  ...
  void canBeDisabled() {
    RateLimitingFilter filter =
        new RateLimitingFilter(new RateLimitProperties(false, 1, Duration.ofMinutes(1)), clock);
    AtomicInteger calls = new AtomicInteger();
    WebFilterChain chain =
        exchange -> {
          calls.incrementAndGet();
