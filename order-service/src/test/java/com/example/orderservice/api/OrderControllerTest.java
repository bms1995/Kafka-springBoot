...
class OrderControllerTest {
  ...
  void createOrderGeneratesOrderIdWhenMissing() {
    ...
    CreateOrderRequest request =
        new CreateOrderRequest(
            " ", "Keyboard", 1, BigDecimal.valueOf(79.90), "customer@example.com");
    when(orderService.createOrder(request)).thenReturn("generated-order-id");

    String response = controller.createOrder(request);

    assertThat(response)
        .isEqualTo("Order created and queued for Kafka with ID: generated-order-id");
  }
  ...
  void getOrderEventsReturnsTimeline() {
    OrderController controller = new OrderController(orderService);
    OrderEventResponse event =
        new OrderEventResponse(
